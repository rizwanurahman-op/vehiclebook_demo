...
export const getExchanges = async (req: Request, res: Response): Promise<void> => {
    ...
    const result = await es.getExchanges({ collection, dateFrom, dateTo, page, limit }, (req as any).adminId!);
...
export const getExchangeStats = async (req: Request, res: Response): Promise<void> => {
    ...
    const stats = await es.getExchangeStats({ dateFrom, dateTo, collection, adminId });
...
export const exportExchanges = async (req: Request, res: Response): Promise<void> => {
    ...
    const result = await es.getExchanges({ collection, dateFrom, dateTo, page: 1, limit: 10_000 }, adminId);
