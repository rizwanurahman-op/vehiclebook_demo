...
export const createLender = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
        const lender = await lenderService.create(req.body, req.adminId!);
...
export const listLenders = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
        const { data, meta } = await lenderService.list(req.query as Record<string, string>, req.adminId!);
...
export const getLender = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
        const lender = await lenderService.getById(req.params.id as string, req.adminId!);
...
export const updateLender = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
        const lender = await lenderService.update(req.params.id as string, req.body, req.adminId!);
...
export const deleteLender = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
        const lender = await lenderService.softDelete(req.params.id as string, req.adminId!);
...
export const restoreLender = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
        const lender = await lenderService.restore(req.params.id as string, req.adminId!);
...
export const hardDeleteLender = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
        await lenderService.hardDelete(req.params.id as string, req.adminId!);
...
export const getLenderStats = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        const stats = await lenderService.getStats(req.adminId!, { status, search, dateFrom, dateTo });
...
export const exportLenders = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        const data = await lenderService.exportAll(req.adminId!, { status, search, dateFrom, dateTo });
...
export const exportLenderStatement = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        const lenderDoc = await lenderService.getById(lenderId, req.adminId!) as unknown as {
        ...
        const [invResult, repResult] = await Promise.all([
            investmentService.exportAll({ lenderId } as Record<string, string>, req.adminId!),
            repaymentService.exportAll({ lenderId }, req.adminId!),
        ]);

        const investments = invResult as { investmentId: string; date: Date; amountReceived: number; mode: string; referenceNo?: string; notes?: string; }[];
        const repayments = repResult as { repaymentId: string; date: Date; amountPaid: number; mode: string; repaymentType?: string; referenceNo?: string; remarks?: string; }[];

        if (format === "pdf") {
            const profile = {
                lenderId: lenderDoc.lenderId ?? "-", name: lenderDoc.name, phone: lenderDoc.phone,
                address: lenderDoc.address, remarks: lenderDoc.remarks, isActive: lenderDoc.isActive,
                totalBorrowed: lenderDoc.totalBorrowed, totalRepaid: lenderDoc.totalRepaid,
                totalProfit: lenderDoc.totalProfit, balancePayable: lenderDoc.balancePayable,
            };
        ...
        if (format === "investments-csv") {
            ...
            const rows = investments.map(i => [i.investmentId, new Date(i.date).toLocaleDateString("en-IN"), i.amountReceived, i.mode, i.referenceNo ?? "", i.notes ?? ""].map(esc).join(","));
            const csv = [headers.map(esc).join(","), ...rows].join("\r\n");
            res.setHeader("Content-Type", "text/csv; charset=utf-8");
        ...
        const rows = repayments.map(r => [r.repaymentId, new Date(r.date).toLocaleDateString("en-IN"), r.repaymentType ?? "Principal", r.amountPaid, r.mode, r.referenceNo ?? "", r.remarks ?? ""].map(esc).join(","));
        const csv = [headers.map(esc).join(","), ...rows].join("\r\n");
        res.setHeader("Content-Type", "text/csv; charset=utf-8");
